...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
         ...
         */
        std::vector<JSAMPLE> jsamples(row_stride);
        for (int i = 0; i < image.GetWidth(); ++i) {
            jsamples[0 + 3 * i] = JSAMPLE(image.GetLine(cinfo.next_scanline)[i].r);
            jsamples[1 + 3 * i] = JSAMPLE(image.GetLine(cinfo.next_scanline)[i].g);
            jsamples[2 + 3 * i] = JSAMPLE(image.GetLine(cinfo.next_scanline)[i].b);
        }

        row_pointer[0] = &jsamples[0];
